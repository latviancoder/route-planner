...
function useSortableElement() {
  ...
  const { addNode, removeNode } = useContext(SortableContext);

  useEffect(() => {
    addNode(ref.current);
    return () => {
      removeNode(ref.current);
    }
  }, []);
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'REMOVE_NODE':
      return produce(state, draft => {
        draft.nodes = draft.nodes.filter(e => e !== action.payload);
      });
    case 'DRAG_START':
      return produce(state, draft => {
...
function Sortable(props) {
  ...
  function startDragging() {
    ...
    state.duplicateNode.style.color = `#7DB434`;
